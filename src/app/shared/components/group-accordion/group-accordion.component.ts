  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  ngAfterViewInit(): void {
    ...
    const accHeads = this.accGroup.nativeElement.querySelectorAll('.accHead');
    this.listenerSub = this.listener.subscribe((accInd) => {
      if (accInd > -1) {
        accHeads.forEach((acc, i) => {
          if (i != accInd && !acc.classList.contains('cusCollapsed')) {
            (acc.querySelector('.accToggle') || acc).click();
          }
        });
      }
    });
