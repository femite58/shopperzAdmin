...
export class PaginationComponent implements OnInit, AfterViewInit {
  ...
  get countListener() {
    if (this.initialCount != this.counts) {
      this.removeAll();
      this.initialCount = this.counts;
    }
    return '';
  }
  ...
  private genPageNum() {
    let firstPage: HTMLElement =
      this.paginationPar.nativeElement.querySelector('.pages');
    ...
    this.setActivePage();
    this.numTexts =
      this.paginationPar.nativeElement.querySelectorAll('.numText');
  ...
  get navPageList() {
    let pageArray = Array(Math.ceil(this.counts / this.limit))
      .fill(0)
      .map((c, i) => {
        return i + 1;
      });
  ...
  displayCheck(): void {
    this.pages.forEach((each, i) => {
      if (
        (this.page - (i + 1) <= 2 && this.page - (i + 1) > 0) ||
