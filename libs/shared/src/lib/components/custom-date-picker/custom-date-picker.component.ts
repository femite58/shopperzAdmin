...
export class CustomDatePickerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.selected = this.fc.value ? Date.parse(this.fc.value) : this.selected;
  ...
  ctrl(n) {
    ...
    if (this.view == 'day') {
      ...
      let date = new Date(d.getFullYear() + 16 * n, d.getMonth(), d.getDate());
      this.genYears(date);
    }
  }
    ...
    for (let i = 0; i < 12; i++) {
      ...
      this.months.push(
        new Date(
          d.getFullYear(),
          i,
          tempD.getMonth() == chk.getMonth() ? d.getDate() : chk.getDate()
        ).getTime()
      );
    }
  }
  ...
  setDay(d) {
    if (new Date(d).getMonth() != new Date(this.viewDate).getMonth()) return;
    this.selected = d;
  ...
  genDaysMonth(date) {
    ...
    while (
      this.dateS.getWeekDay(new Date(this.monthDays.slice(-1)[0])) != 'Saturday'
    ) {
      this.monthDays.push(this.monthDays.slice(-1)[0] + 24 * 60 * 60 * 1000);
    }
  }
  ...
  genWeekDays() {
    ...
    while (
      this.dateS.getWeekDay(new Date(this.weekDays.slice(-1)[0])) != 'Saturday'
    ) {
      this.weekDays.push(this.weekDays.slice(-1)[0] + 24 * 60 * 60 * 1000);
    }
  }
}
