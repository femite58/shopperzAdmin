...
export class GraphComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private responsive() {
    this.genPoints = [];
    const svgw = +getComputedStyle(this.svgEl.nativeElement).width.replace(
      'px',
      ''
    );
    const svgh = +getComputedStyle(this.svgEl.nativeElement).height.replace(
      'px',
      ''
    );
  ...
  private initiateGrouping() {
    ...
    let firstDate =
      Date.parse(this.groupedArr.slice(-1)[0].date) - 29 * 24 * 60 * 60 * 1000;
