...
export class BarChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private dateService: MyDateService) {}
  ...
  ngAfterViewInit() {
    this.eachSingles =
      this.barChartPar.nativeElement.querySelectorAll('.eachSingle');
    this.responsive();
    this.winResEv(true);
    this.setInitial();
    this.scrollCheck();
  ...
  private responsive() {
    let parw = +getComputedStyle(this.barChartPar.nativeElement).width.replace(
      'px',
      ''
    );
    ...
    this.barChartPar.nativeElement.style.height = `${
      finalh < 192.11 ? 192.11 : finalh
    }px`;
  ...
  private getDuration() {
    this.groupedArr.forEach((each) => {
      let eachDay = this.dateService.getShortWeekDay(
        new Date(each.date).getDay()
      );
  ...
  private initiateGrouping() {
    ...
    if (this.groupedArr.length) {
      let dayDiff = Math.floor(
        (Date.now() - Date.parse(this.groupedArr.slice(-1)[0].date)) /
          (1000 * 60 * 60 * 24)
      );
    ...
    let firstDate =
      Date.parse(this.groupedArr.slice(-1)[0].date) - 6 * 24 * 60 * 60 * 1000;
    let firstDateObj = new Date(firstDate);
    let [fy, fm, fd] = [
      firstDateObj.getFullYear(),
      firstDateObj.getMonth() + 1,
      firstDateObj.getDate(),
    ];
    let [iy, im, id] = this.groupedArr[0].date.split('-');
    while (fy != iy || fm != im || fd != id) {
      let insDate = new Date(
        Date.parse(this.groupedArr[0].date) - 24 * 60 * 60 * 1000
      );
      let [y, m, d] = [
        insDate.getFullYear(),
        insDate.getMonth() + 1,
    ...
    for (let i = 0; i < rawData.length; i++) {
        ...
        dateExist = this.groupedArr.filter((each) => {
          ...
          if (each.data[I]) {
            innerExist = each.data[I].filter((each2) => {
              return (
                each2.created_at.split(' ')[0] ==
                rawData[i].created_at.split(' ')[0]
              );
            });
          }
      ...
      if (I == 0) {
        ...
        if (!dateExist) {
          ...
          if (matchInd != -1) {
            this.groupedArr[matchInd].data.push(
              rawData.filter((each) => {
                let currTransDate = rawData[i].created_at.split(' ')[0];
                return currTransDate == each.created_at.split(' ')[0];
              })
